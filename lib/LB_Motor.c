...
void Init_MotorSpeed()
{
	...
	P3M4 = 0x68;			        	//P15ÉèÖÃÎª´øSMTÉÏÀ­ÊäÈë
	P3M5 = 0x68;			        	//P16ÉèÖÃÎª´øSMTÉÏÀ­ÊäÈë

							//INT16	17
 	PITS4 |= 0x0F;						

    PINTE2 |= 0x03;						//Ê¹ÄÜINT17 16
...
void InitMotorIO(void)
{
    ...
    P1M1 = 0xC2;                        //P11ÉèÖÃÎªÍÆÍìÊä³ö
    P1M2 = 0xC2;                        //P12ÉèÖÃÎªÍÆÍìÊä³ö
    P1M3 = 0xC2;                        //P13ÉèÖÃÎªÍÆÍìÊä³ö
    P1M4 = 0xC2;                        //P14ÉèÖÃÎªÍÆÍìÊä³ö
...
{
    P3M0 = 0xC2;                        //P34ÉèÖÃÎªÍÆÍìÊä³ö
    P3M1 = 0xC2;                        //P35ÉèÖÃÎªÍÆÍìÊä³ö
    P3_0=1;
    P3_1=1;

	PWM1_MAP = 0x30;					//PWM1Í¨µÀÓ³ÉäP34¿Ú
	PWM11_MAP = 0x31;					//PWM11Í¨µÀÓ³ÉäP35¿Ú
    PWM1C = 0x01;						//PWM1¸ßÓÐÐ§£¬PWM11¸ßÓÐÐ§£¬Ê±ÖÓ8·ÖÆµ 
    PWMM |= 0x20;						//PWM1¹¤×÷ÓÚ						

	//¶ÀÁ¢Ä£Ê½ÏÂ£¬PWM0ºÍPWM01¹²ÓÃÒ»¸öÖÜÆÚ¼Ä´æÆ÷
	//PWM0µÄÕ¼¿Õ±Èµ÷½ÚÊ¹ÓÃ			PWM0×éµÄÕ¼¿Õ±È¼Ä´æÆ÷
	//PWM01µÄÕ¼¿Õ±Èµ÷½ÚÊ¹ÓÃ			PWM0×éµÄËÀÇø¼Ä´æÆ÷

	//ÖÜÆÚ¼ÆËã 	= 0x03ff / (Fosc / PWM·ÖÆµÏµÊý)		£¨Fosc¼ûÏµÍ³Ê±ÖÓÅäÖÃµÄ²¿·Ö£©
	//			= 0x03ff / (16000000 / 8)			
	// 			= 1023   /2000000
	//			= 511.5us		   		Ô¼1.955kHz

	PWM1PH = 0x00;						//ÖÜÆÚ¸ß4Î»ÉèÖÃÎª0x03
	PWM1PL = 0xff;						//ÖÜÆÚµÍ8Î»ÉèÖÃÎª0xFF

	//Õ¼¿Õ±È¼ÆËã= 0x0155 / (Fosc / PWM·ÖÆµÏµÊý)		£¨Fosc¼ûÏµÍ³Ê±ÖÓÅäÖÃµÄ²¿·Ö£©
	//			= 0x0155 / (16000000 / 8)			
	// 			= 341 	 / 2000000
	//			= 170.5us		   Õ¼¿Õ±ÈÎª 170.5/511.5 = 33.3%

	PWM1DH = 0x00;						//PWM1¸ß4Î»Õ¼¿Õ±È0x01
	PWM1DL = 0x00;						//PWM1µÍ8Î»Õ¼¿Õ±È0x55
	PWM1DTH = 0x00;						//PWM11¸ß4Î»Õ¼¿Õ±È0x01
	PWM1DTL = 0x00;						//PWM11µÍ8Î»Õ¼¿Õ±È0x55
	PWMEN |= 0x22;						//Ê¹ÄÜPWM1ÒÔ¼°PWM11
 ...
 void InitMotorLeftRetreat(void)
{
    ...
    P1_3=0;
    P1_4=0;
	PWM0_MAP = 0x13;					//PWM0Í¨µÀÓ³ÉäP14¿Ú
    PWM0C = 0x00;						//PWM0¸ßÓÐÐ§£¬PWM01¸ßÓÐÐ§£¬Ê±ÖÓ8·ÖÆµ 
    PWMM |= 0x10;						//PWM0¹¤×÷ÓÚ»¥²¹Ä£Ê½						
	...
	PWM0PH = 0x01;						//ÖÜÆÚ¸ß4Î»ÉèÖÃÎª0x03
	PWM0PL = 0x0;						//ÖÜÆÚµÍ8Î»ÉèÖÃÎª0xFF
	...
	PWM0DH = 0x00;						//PWM0¸ß4Î»Õ¼¿Õ±È0x01
	PWM0DL = 0x60;						//PWM0µÍ8Î»Õ¼¿Õ±È0x55
	PWMEN |= 0x01;						//Ê¹ÄÜPWM0ÒÔ¼°PWM01
 ...
 void InitMotorRightRetreat(void)
{
	...
	PWM01_MAP = 0x12;					//PWM01Í¨µÀÓ³ÉäP12¿Ú
    PWM0C = 0x00;						//PWM0¸ßÓÐÐ§£¬PWM01¸ßÓÐÐ§£¬Ê±ÖÓ8·ÖÆµ 
    PWMM |= 0x10;						//PWM0¹¤×÷ÓÚ»¥²¹Ä£Ê½						
	...
	PWM0DTH = 0x00;						//PWM01¸ß4Î»Õ¼¿Õ±È0x01
	PWM0DTL = 0x60;						//PWM01µÍ8Î»Õ¼¿Õ±È0x55
	PWMEN |= 0x10;						//Ê¹ÄÜPWM0ÒÔ¼°PWM01
 ...
 void InitMotorLeftForward(void)
{
    ...
    P1_3=0;
    P1_4=0;
	PWM0_MAP = 0x14;					//PWM0Í¨µÀÓ³ÉäP14¿Ú
    PWM0C = 0x00;						//PWM0¸ßÓÐÐ§£¬PWM01¸ßÓÐÐ§£¬Ê±ÖÓ8·ÖÆµ 
    PWMM |= 0x10;						//PWM0¹¤×÷ÓÚ»¥²¹Ä£Ê½						
 ...
 void InitMotorRightForward(void)
{
	...
	PWM01_MAP = 0x11;					//PWM01Í¨µÀÓ³ÉäP12¿Ú
    PWM0C = 0x00;						//PWM0¸ßÓÐÐ§£¬PWM01¸ßÓÐÐ§£¬Ê±ÖÓ8·ÖÆµ 
    PWMM |= 0x10;						//PWM0¹¤×÷ÓÚ»¥²¹Ä£Ê½						
 ...
 void ReadLeftPulsed(void)
{
	...
	if((LeftMoveMotorData.MotorMode&0x03)==1)
	{
		...
		if(LeftMoveMotorData.NowPulsed>=(LeftMoveMotorData.MovePulsed+80))
		{
			...
			{
				LeftMoveMotorData.Flag=1; //å·¦å³é©¬è¾¾è¿è¡Œä¸åŒ¹é…ï¼Œæ ‡å¿—ç¬¦
				LeftStop();
			}
		}
	}
