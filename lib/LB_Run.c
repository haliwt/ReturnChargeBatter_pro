...
//			ADCtl = 0;
            ...
            timeCircle= 22;// н?μ?±??timeCircle= 50;//
		...
		case 0x1e: //left turn run 90 degree
		    if(IMP>0)
			{
					NoImpSecond=0;
					RunStep=0x3;
...
void battVoltDetect(void)
{
	...
	if(Voltage < 0x3b3 && Voltage > 0x300 ){//if(Voltage < 0x3F6 && Voltage > 0x3a0 ){  //0x3f6??|7Vμ瑹  ￡?0x3a0??|6.1Vμ瑹 ,0X3d0 ??|3.85Vμ瑹￡¨?А?μ??瑹￡?
		if(RunMode == 5)
			return;
